#!/usr/bin/env python3
"""
Unified evaluation script for ablation study: Direct LLM API code generation.

This script evaluates code generation by directly calling LLM API
without using the multi-agent collaboration framework.

Steps:
1. Generate code and run tests (evaluate.py)
2. Generate final report (reuse generate_report.py from parent directory)
"""
...
DEFAULT_CONFIG = {
    'experiment': 'ablation_direct_llm',
    'model': 'deepseek',
    'topn': 1,
    'temperature': '0.3',
...
# Import evaluation function from this directory
from evaluate import run_evaluation

# Import report generation from parent directory
...
def run_step1(experiment, model, topn, temperature, config_path, max_problems=None, random_seed=None, prompt_type='', selected_problem_names=None):
    """Step 1: Generate code and run tests using direct LLM API"""
    output_file = f"{experiment}_model_{model}_topn_{topn}_temperature_{temperature}.0.log_0"
    temperature_float = float(temperature)
    run_evaluation(output_file, topn, config_path, temperature_float, max_problems, random_seed, prompt_type, selected_problem_names)
    return output_file
...
def main():
    parser = argparse.ArgumentParser(
        description="Ablation study evaluation: Direct LLM API code generation",
...
Examples:
  ...
  python main_evaluate.py -e ablation_direct_llm -m deepseek -n 1 -t 0.3 -c config/agent_config.yaml --max-problems 10 --random-seed 42
    ...
    parser.add_argument("-c", "--config", type=str, help="LLM config file path", default=None)
